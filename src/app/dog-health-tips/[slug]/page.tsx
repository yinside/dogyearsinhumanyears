...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      description: article.metaDescription,
      type: 'article',
      url: `${baseUrl}/dog-health-tips/${slug}`,
    },
    alternates: { canonical: `${baseUrl}/dog-health-tips/${slug}` },
  };
}

export default async function DogHealthArticle({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const article = getArticle(slug);
  if (!article) notFound();
    ...
    <>
      ...
      <div className="container-main" style={{ paddingTop: '15px', paddingBottom: '30px' }}>
        <div className="breadcrumb">
          <Link href="/">Home</Link> &raquo; <Link href="/dog-health-tips">Dog Health Tips</Link> &raquo; {article.title}
        </div>
        ...
        <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
          <div style={{ flex: '1 1 450px' }}>
            <div className="section-box">
              ...
              <div style={{ fontSize: '12px', color: '#777', marginBottom: '15px' }}>
                <span style={{ background: '#f0fff0', border: '1px solid #c5e5c5', padding: '2px 8px' }}>{article.category}</span>
          ...
          <div style={{ flex: '1 1 280px', minWidth: '280px' }}>
            <div className="widget">
              <h3>Dog Health Tips</h3>
              <ul>
                {getAllSlugs().map((s) => {
                  const a = getArticle(s);
                  if (!a) return null;
                  return <li key={s}><Link href={`/dog-health-tips/${s}`}>{a.title}</Link></li>;
                })}
              </ul>
            </div>
            ...
            <div className="widget">
              ...
              <ul>
                ...
                <li><Link href="/puppy-care">Puppy Care Articles</Link></li>
                <li><Link href="/senior-dog-care">Senior Dog Care</Link></li>
              </ul>
            </div>
          </div>
        </div>
