...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      description: article.metaDescription,
      type: 'article',
      url: `${baseUrl}/puppy-care/${slug}`,
    },
    alternates: { canonical: `${baseUrl}/puppy-care/${slug}` },
  };
}

export default async function PuppyCareArticle({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const article = getArticle(slug);
  if (!article) notFound();
    ...
    <>
      ...
      <div className="container-main" style={{ paddingTop: '15px', paddingBottom: '30px' }}>
        <div className="breadcrumb">
          <Link href="/">Home</Link> &raquo; <Link href="/puppy-care">Puppy Care</Link> &raquo; {article.title}
        </div>
        ...
        <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
          <div style={{ flex: '1 1 450px' }}>
            <div className="section-box">
              ...
              <div style={{ fontSize: '12px', color: '#777', marginBottom: '15px' }}>
                <span style={{ background: '#fff8f0', border: '1px solid #e5d0b5', padding: '2px 8px' }}>{article.category}</span>
              ...
              <div className="article-content">
                ...
                <div className="related-links">
                  ...
                  <Link href="/dog-health-tips/dog-nutrition-guide">Dog Nutrition Guide</Link> |
          ...
          <div style={{ flex: '1 1 280px', minWidth: '280px' }}>
            <div className="widget">
              <h3>Puppy Care Guides</h3>
              <ul>
                {getAllSlugs().map((s) => {
                  const a = getArticle(s);
                  if (!a) return null;
                  return <li key={s}><Link href={`/puppy-care/${s}`}>{a.title}</Link></li>;
                })}
              </ul>
            </div>
